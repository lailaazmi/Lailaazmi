...
#define LOG_PATH @"/var/mobile/Library/Safari/menustyle.log"
...
@implementation UIMenuController (MenuStyle) // {{{
/// Plugin registration function
- (id<MSMenuItem>)ms_registerAction:(SEL)action title:(NSString *)title canPerform:(SEL)canPerform
{
    NSLog(@"registed: %@, %@, %@", title, NSStringFromSelector(action), NSStringFromSelector(canPerform));
...
static NSString *Invoke(UIResponder *self, NSString *(*function)(id<UITextInput> view))
{
    ...
    if ([self isKindOfClass:%c(UIWebView)]) {
        ...
        if ([cv isKindOfClass:%c(WKContentView)]) {
            ...
            NSLog(@"WKWebView: %@, %@", result, self);
        } else {
      // https://github.com/lailaazmi/MenuStyle/issues/1
 ...
 NSLog(@"MenuStyle: currently not support for PDF");
        ...
        result = (*function)(v);
        NSLog(@"UITextInput: %@", result);
    }
    return result;
}
// }}}
@implementation UIResponder (MenuStyle) // {{{
...
- (void)updateAvailableButtons
{
    ...
    for (NSString *item in subpaths) {
        ...
        if ([ext isEqualToString:@"bundle"]) {
            ...
            if (!bundle.isLoaded) {
                ...
                NSLog(@"loaded bundle: %@, %d, %@", bundle, result, e);
    ...
    NSLog(@"MenuStyle bundle loading error: %@", e);
                }
            }
        } else if ([ext isEqualToString:@"dylib"]) {
            NSString *path = [NSString stringWithFormat:@"%@%@", PLUGINS_DIR_PATH, item];
            const char *filepath = path.UTF8String;
            NSLog(@"loading dylib: %@", path);
            ...
            if (handle && bfCount + 1 == afCount) {
                ...
                NSLog(@"MenuStyle dylib open error.");
            }
        }
    }
    ...
    for (MSMenuPlugin *plugin in m.plugins) {
        ...
        [a addObject:desc];
        NSLog(@"added buttonDescription: %@", plugin.title);
    }
    ...
    for (MSMenuPlugin *plugin in m.plugins) {
        if (action == plugin.action) {
            ...
            if (![self respondsToSelector:plugin.canPerform]) {
                NSLog(@"NoSelector: %@, %@", self, sender);
                return NO;
            }
            if (!plugin.isEnabled) {
                NSLog(@"Disabled: %@, %@", self, sender);
                return NO;
            }
            ...
            NSLog(@"canPerform: %d", returnValue);
            return returnValue;
        }
    }
